...
- Auth files with an account-level `proxy_url` remain normal business-scheduling candidates. For proactive quota GETs, manual refresh, and lazy-window maintenance, the management UI can opt into a loopback CPA management bridge. The bridge is off by default, accepts only a literal `127.0.0.1`/`[::1]` management origin, keeps the management key write-only, and sends the validated account proxy explicitly through CPA `/v0/management/api-call`; invalid or failed proxy paths never fall back to the plugin's ordinary host callback.
- “Allow window activation through the management API” is a separate switch and defaults off. With only the bridge enabled, proxy auths may refresh quota but cannot send an activation POST. Proxy activation additionally requires the existing global activation switch and every existing lazy-window, concurrency, retry, and five-attempt guard. Auths without an account proxy keep the original callback path and are unaffected by this new switch.
