...
3. **做限制** — 单 key 的 RPM、最大在途请求并发、可选 UTC 今日/近 7 个 UTC 自然日美元额度，并可限制某个 auth 文件的全局受控并发。
...
- “今日”是当前 **UTC 自然日**；七日用量是今日加此前 6 个 UTC 日期，不是另起点的 168 小时周期。UTC 午夜只滚出最早一天，因此七日用量会包含今日用量。
- `daily_limit_usd` 与 `weekly_limit_usd` 使用和管理页、公开 Lookup 相同的账目。额度仍在请求完成后记账，并发请求可能让最终金额略微超过上限。
- 从 v0.8.x 首次升级时，插件先创建不覆盖的 `cpa-key-policy-state.json.pre-usage-v2.<timestamp>.bak`，只迁移能确认属于当天的旧用量，并提示“七日历史不完整”；经过 6 个新的 UTC 日界后提示自动消失。旧独立周累计只保留在备份中。
- Key 列表中的“重置 RPM”和“重置用量”是两个独立操作。“重置用量”只清目标派生 Key 的金额、Token、请求数和别名明细；额度上限、RPM 计数、绑定、并发、亲和与上游账号额度均不变。在途请求随后完成时仍会重新计入。CPA 原生 Key 不支持该操作。
- 重置结果在返回成功前同步落盘。不要让旧版插件直接写新的 v2 主状态；回滚时应停止写入并恢复对应的迁移前备份。升级后新增的账目无法无损降级。
...
- 配置账号级 `proxy_url` 的 auth file 仍正常参与业务调度。对于主动 quota GET、手动刷新和懒窗口维护，可在管理页面选择启用本机 CPA 管理桥接。桥接默认关闭，只接受字面量 `127.0.0.1`/`[::1]` 管理基址，管理密钥仅写入不回显，并通过 CPA `/v0/management/api-call` 显式传递已校验的账号代理；代理非法或失败时不会退回插件普通宿主回调直连。
- “允许通过管理接口激活窗口”是独立开关，默认关闭。只开启桥接时，代理账号可以刷新额度，但不能发送激活 POST；代理激活还必须同时开启原有全局自动激活，并继续通过懒窗口、并发、重试和总计五次等既有门禁。没有账号代理、继续使用旧回调路径的账号不受这个新开关影响。
...
自助页只使用 `Authorization: Bearer <key>`。插件派生 Key 显示自己的 UTC 今日/近 7 个 UTC 自然日用量、调用/Token 汇总和当前并发；若已配置明确账号绑定，还会显示权限交集内的匿名 Codex 额度缓存。页面普通“刷新”仍只读缓存；管理员需另外开启 `allow_quota_refresh`，持有人才能逐账号刷新额度。显式导入并启用的 CPA 原生 Key 可以聚合查看全部插件派生 Key 的用量，但不会得到上游账号额度或刷新权限。纯插件无法识别未导入的宿主 `api-keys`；接口不接受 URL 中的 key、`key_id` 或 auth ID。

| 区域 | 用途 |
|------|------|
| Keys | 创建/编辑/轮换/删除 key；绑模型或别名；RPM、额度与单 Key 用量重置 |
